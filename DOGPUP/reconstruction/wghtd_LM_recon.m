function [new_mu,iter,err,errPerc,errDel] = wghtd_LM_recon(mesh,data_m,gates,pos_s,lam,type,floor)
...
if ~isempty(pos_s)
    spots = target_spots(mesh,pos_s,mesh.dxyz/2,'gauss');
    if strcmp(type,'full')
...
while 1
    ...
    d_mu = mesh.g2m*(reshape(update,size(mesh.m2g,1),[])); % interpolate to mesh
    ...
    % update DOGPUP mesh
    if strcmp(type,'full') 
        mesh = update_properties(mesh,new_mu);
    else
